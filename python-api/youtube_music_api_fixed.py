...
def find_track():
    ...
    if not query:
        return jsonify({
            'error': 'Se requiere un parámetro de búsqueda: title y artist, o q'
        }), 400
                        ...
                        if 'params' in category:
                            try:
                                ...
                                if mood_playlists and 'playlists' in mood_playlists:
                                    logger.info(f"[RASTREO-PLAYLIST] Categoría '{category.get('title', '')}': {len(mood_playlists['playlists'])} playlists")
                                        ...
                                        if 'browseId' in playlist:
                                            try:
                                                ...
                                                if 'tracks' in playlist_tracks:
                                                    logger.info(f"[RASTREO-PLAYLIST] Obtenidas {len(playlist_tracks['tracks'])} tracks de mood")
                                                    ...
                                                    for track in playlist_tracks['tracks']:
                                                        if 'videoId' in track:
                                                            track_data = {
                                                                'id': track['videoId'],
                                                                'title': track.get('title', 'Sin título'),
                                                                'artist': ', '.join([a.get('name', '') for a in track.get('artists', [])]) if 'artists' in track else 'Artista desconocido',
                                                                'thumbnail': get_best_thumbnail(track.get('thumbnails', [])),
                                                ...
                                                logger.error(f"[RASTREO-PLAYLIST] Error obteniendo playlist de mood: {str(playlist_error)}")
