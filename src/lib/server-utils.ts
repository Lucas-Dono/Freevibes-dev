...
export async function isNodeServerAvailable(): Promise<boolean> {
  ...
  try {
    ...
    // Verificar que la respuesta sea del servidor Node (si devuelve server: 'node')
    return data?.status === 'OK' && data?.server === 'node';
  } catch (error: any) {
...
export async function isPythonApiAvailable(): Promise<boolean> {
  ...
  try {
    ...
    const data = await response.json();
    // Verificar que la respuesta sea del servidor Python (si devuelve server: 'python')
    return data?.status === 'OK' && data?.server === 'python';
  } catch (error: any) {
