...
export function getCountryCode(): string {
  try {
    ...
    const savedRegion = typeof window !== 'undefined' && window.localStorage ? localStorage.getItem('user_region') : null;
...
export function setUserRegion(countryCode: string): void {
  try {
    if (typeof window !== 'undefined' && window.localStorage && countryCode && countryCode.length === 2) {
